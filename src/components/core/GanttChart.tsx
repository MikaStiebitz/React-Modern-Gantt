...
const GanttChart = forwardRef<GanttChartRef, GanttChartProps>(({
    ...
    const scrollToNow = (mode: ViewMode, unitWidth: number) => {
        console.log("scrollToNow called:", { mode, unitWidth, focusMode, hasContainer: !!scrollContainerRef.current });

        if (!scrollContainerRef.current) {
            console.log("scrollToNow: no container");
            return;
        }
        if (!focusMode) {
            console.log("scrollToNow: focusMode is false");
            return;
        }
        ...
        console.log("scrollToNow:", { mode, unitWidth, targetScroll, timeUnitsLength: newTimeUnits.length });
    ...
    // Scroll to "now" when focus mode is enabled and timeline updates
    useEffect(() => {
        if (focusMode && scrollContainerRef.current && currentUnitIndex >= 0 && timeUnits.length > 0) {
    ...
    }, [focusMode, activeViewMode, viewUnitWidth, currentUnitIndex, timeUnits.length]);

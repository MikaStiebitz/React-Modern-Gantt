...
const DemoViewModes: React.FC<DemoViewModesProps> = ({ darkMode }: DemoViewModesProps) => {
    ...
    const [demoType, setDemoType] = React.useState<"complex" | "yearLong" | "hourly">("complex");
    ...
    const handleDemoTypeChange = (type: "complex" | "yearLong" | "hourly") => {
        ...
        if (type === "complex") {
            ...
            if (viewMode === ViewMode.DAY || viewMode === ViewMode.HOUR || viewMode === ViewMode.MINUTE) {
                setViewMode(ViewMode.MONTH);
            }
        } else if (type === "hourly") {
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: "20px" }}>
                <p>
                    This demo showcases different view modes (Minute, Hour, Day, Week, Month, Quarter, Year).
                    {demoType === "hourly" &&
                        " 🕐 Try the Hour view to see hourly tasks with precise time tracking and drag the progress handles!"}
                    {demoType === "complex" && " 📊 Use the progress bar handles to adjust task completion percentage."}
                    {demoType === "yearLong" && " 📅 Perfect for viewing long-term projects across the year."}
                </p>
            </div>
